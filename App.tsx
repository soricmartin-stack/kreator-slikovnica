    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 pb-20">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-10">
        ...
        {step === AppStep.Scenes && (
          <div className="space-y-10 animate-in fade-in duration-500">
            <div className="flex items-center justify-between">
              <div><h2 className="text-3xl font-bold font-handwriting">Book Storyboard</h2><p className="text-slate-500">Edit page text and regenerate images from narrative or visual prompts.</p></div>
              <div className="flex gap-4">
                <button onClick={downloadAll} className="px-6 py-2 bg-white border border-slate-200 text-slate-700 rounded-xl font-bold hover:bg-slate-50 transition-all flex items-center gap-2">
                  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" /></svg>
                  Download All
                </button>
                <button onClick={() => window.location.reload()} className="px-6 py-2 bg-slate-200 text-slate-700 rounded-xl font-bold hover:bg-slate-300 transition-all">Start Over</button>
              </div>
            </div>
